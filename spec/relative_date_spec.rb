...
RSpec.describe RelativeDate do
  it "has a version number" do
    expect(RelativeDate::VERSION).not_to be nil
  end
  ...
  describe ".parse" do
    ...
    context "with weekday patterns" do
      it "parses '2 Mondays ago'" do
        result = RelativeDate.parse("2 Mondays ago", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 1)) # January 1, 2024
      end

      it "parses '3 Fridays from now'" do
        result = RelativeDate.parse("3 Fridays from now", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 2, 2)) # February 2, 2024
      end

      it "parses '1 Sunday ago'" do
        result = RelativeDate.parse("1 Sunday ago", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 14)) # January 14, 2024
      end

      it "handles singular form 'Monday'" do
        result = RelativeDate.parse("1 Monday ago", reference_date: reference_date)
    ...
    context "with next/last weekday patterns" do
      it "parses 'next Friday'" do
        result = RelativeDate.parse("next Friday", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 19)) # January 19, 2024
      end

      it "parses 'last Friday'" do
        result = RelativeDate.parse("last Friday", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 12)) # January 12, 2024
      end

      it "parses 'next Monday'" do
        result = RelativeDate.parse("next Monday", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 22)) # January 22, 2024
      end

      it "parses 'last Sunday'" do
        result = RelativeDate.parse("last Sunday", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 14)) # January 14, 2024
      end
    end
    ...
    context "with unit-based patterns" do
      it "parses '3 days ago'" do
        result = RelativeDate.parse("3 days ago", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 12))
      end

      it "parses '2 weeks from now'" do
        result = RelativeDate.parse("2 weeks from now", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 29))
      end

      it "parses '1 month ago'" do
        result = RelativeDate.parse("1 month ago", reference_date: reference_date)
        expect(result).to eq(Date.new(2023, 12, 15))
      end

      it "parses '2 months from now'" do
        result = RelativeDate.parse("2 months from now", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 3, 15))
      end

      it "parses '1 year ago'" do
        result = RelativeDate.parse("1 year ago", reference_date: reference_date)
        expect(result).to eq(Date.new(2023, 1, 15))
      end

      it "parses '2 years from now'" do
        result = RelativeDate.parse("2 years from now", reference_date: reference_date)
    ...
    context "with simple relative patterns" do
      it "parses 'today'" do
        result = RelativeDate.parse("today", reference_date: reference_date)
        expect(result).to eq(reference_date)
      end

      it "parses 'tomorrow'" do
        result = RelativeDate.parse("tomorrow", reference_date: reference_date)
        expect(result).to eq(Date.new(2024, 1, 16))
      end

      it "parses 'yesterday'" do
        result = RelativeDate.parse("yesterday", reference_date: reference_date)
    ...
    context "with invalid input" do
      it "returns nil for unparseable input" do
        result = RelativeDate.parse("invalid input", reference_date: reference_date)
        expect(result).to be_nil
      end

      it "returns nil for empty string" do
        result = RelativeDate.parse("", reference_date: reference_date)
        expect(result).to be_nil
      end
    end
  ...
  describe ".parse!" do
    ...
    it "raises an error for invalid input" do
      expect {
        RelativeDate.parse!("invalid input", reference_date: reference_date)
      }.to raise_error(RelativeDate::Error, "Unable to parse 'invalid input'")
    end
  end
end
